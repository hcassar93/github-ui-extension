...
function openInModal(url, title) {
  ...
  const existing = modalState.tabs.find(tab => tab.url === url);
  if (existing) {
...
function createModalContainer() {
  ...
  modalState.container.id = 'github-ui-ext-modal-container';
  modalState.container.innerHTML = `
    <div id="github-ui-ext-tabs"></div>
    <div id="github-ui-ext-iframe-container"></div>
  `;
  document.body.appendChild(modalState.container);
}

function renderTabs() {
  const tabsContainer = document.getElementById('github-ui-ext-tabs');
  if (!tabsContainer) return;
  
  tabsContainer.innerHTML = modalState.tabs.map(tab => `
    <div class="github-ui-ext-tab ${tab.id === modalState.activeTab ? 'active' : ''}" data-tab-id="${tab.id}">
      <span class="github-ui-ext-tab-title">${tab.title}</span>
      <span class="github-ui-ext-tab-close" data-close-tab="${tab.id}">×</span>
    </div>
  `).join('');
  
  // Add click handlers
  tabsContainer.querySelectorAll('.github-ui-ext-tab').forEach(tabEl => {
  ...
  tabsContainer.querySelectorAll('.github-ui-ext-tab-close').forEach(closeBtn => {
...
function createIframe(tab) {
  const container = document.getElementById('github-ui-ext-iframe-container');
  if (!container) return;
...
function closeTab(tabId) {
  const index = modalState.tabs.findIndex(tab => tab.id === tabId);
  if (index === -1) return;
